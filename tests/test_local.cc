...
DistributedHnsw* global_dhnsw = nullptr;
std::mutex dhnsw_mutex; // For thread-safe access during initialization

int main(int argc, char** argv) {
    gflags::ParseCommandLineFlags(&argc, &argv, true);
    bind_thread_to_cores(0, 0, 144, FLAGS_use_physical_cores_only);
    ...
    global_dhnsw = new DistributedHnsw(dim, num_sub_hnsw, meta_hnsw_neighbors, sub_hnsw_neighbors, num_meta);
    global_dhnsw->build(base_data, 1000);
...
void* client_worker(void* param) {
    ...
    DistributedHnsw* local_hnsw_ptr = nullptr;
    {
        ...
        local_hnsw_ptr = new DistributedHnsw(*global_dhnsw); // Use copy constructor
    }
    DistributedHnsw& local_hnsw = *local_hnsw_ptr;
    ...
    auto run_ef_benchmark = [&](int ef, size_t duration_sec) {
        ...
        while (duration_cast<seconds>(high_resolution_clock::now() - bench_start).count() < (long)duration_sec) {
            ...
            local_hnsw.hierarchicalSearch(current_batch_size, batch_query_data_ptr, branching_k, top_k, 
                                        batch_distances, batch_labels, batch_sub_hnsw_tags, batch_original_index, ef);
            ...
            // Note: hierarchicalSearch already computes original_index, so we don't need to do it manually
