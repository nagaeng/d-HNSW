...
int main(int argc, char** argv) {
    ...
    std::cout << "=== Vector Max Size Information ===" << std::endl;
    std::cout << "std::vector<float>::max_size()  = " << float_vec.max_size() << std::endl;
    std::cout << "std::vector<uint8_t>::max_size() = " << uint8_vec.max_size() << std::endl;
    ...
    std::cout << "Theoretical float vector elements: " << SIZE_MAX / sizeof(float) << std::endl;
    std::cout << "Theoretical uint8 vector elements: " << SIZE_MAX / sizeof(uint8_t) << std::endl;
    
    // 你的需求
    size_t needed_elements = 100000000ULL * 128ULL; // 100M * 128 dim
    std::cout << "\n=== Your Requirements ===" << std::endl;
    std::cout << "Needed elements: " << needed_elements << std::endl;
    std::cout << "Float vector sufficient: " << (needed_elements <= float_vec.max_size() ? "YES" : "NO") << std::endl;
    ...
    std::vector<float> base_data = read_bvecs(FLAGS_dataset_path, dim_, num_,1,100000000);
    ...
    dhnsw.serialize_with_record_with_gap_to_file("dhnsw.idx", offset);
    ...
    int fd = open("dhnsw.idx", O_RDONLY);
    if (fd < 0) {
