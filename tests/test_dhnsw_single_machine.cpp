...
int main(){
    ...
    std::vector<uint8_t> serialized_data = dhnsw.serialize_with_record(offset);
    std::vector<uint8_t> serialized_meta_hnsw = dhnsw.serialize_meta_hnsw();
    std::vector<uint8_t> serialized_offset = dhnsw.serialize_offset(offset);
    // DistributedHnsw dhnsw2 = DistributedHnsw::deserialize(serialized_data);
    ...
    // LocalHnsw dhnsw2(dim, num_sub_hnsw, meta_hnsw_neighbors);
    LocalHnsw dhnsw2;
    faiss::IndexHNSWFlat* meta_hnsw2 = dhnsw2.deserialize_meta_hnsw(serialized_meta_hnsw);
    dhnsw2.set_meta_hnsw(meta_hnsw2);
    std::vector<size_t> offset2 = dhnsw2.deserialize_offset(serialized_offset);
    ...
    dhnsw_idx_t *original_index = new dhnsw_idx_t[top_k * n_query_data];
    dhnsw2.meta_search(n_query_data, query_data.data(), branching_k, meta_distances, meta_labels, sub_hnsw_tosearch);
    ...
    dhnsw2.set_sub_hnsw_cache(DistributedHnsw::deserialize_sub_hnsw_batch(serialized_data, offset2, sub_hnsw_tosearch), sub_hnsw_tosearch);
    dhnsw2.deserialize_set_sub_hnsw_batch(serialized_data, offset2, sub_hnsw_tosearch);
    dhnsw2.set_local_sub_hnsw_tag(local_sub_hnsw);
    ...
    dhnsw_idx_t *labels = new dhnsw_idx_t[top_k * n_query_data];
    float *distances = new float[top_k * n_query_data];
    dhnsw2.sub_search_each(n_query_data, query_data.data(), branching_k, top_k, distances, labels, sub_hnsw_tosearch, sub_hnsw_tags);
