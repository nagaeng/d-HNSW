...
int main(int argc, char** argv) {
    ...
    if (unique_id.empty()) {
        // Auto-generate from PID + timestamp to ensure uniqueness across nodes
...
void* client_worker(void* param) {
    ...
    auto run_ef_benchmark = [&](int ef, size_t duration_sec) {
        ...
        while (duration_cast<seconds>(high_resolution_clock::now() - bench_start).count() < (long)duration_sec) {
            ...
            local_hnsw.meta_search(current_batch_size, batch_query_data_ptr, branching_k, 
            ...
            total_compute_time += batch_result.first;
            total_network_latency += batch_result.second;
