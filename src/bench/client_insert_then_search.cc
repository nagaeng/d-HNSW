// client_insert_then_search.cc
// First inserts data, then benchmarks search latency
...
DEFINE_int32(num_vectors_to_insert, 10000, "Number of vectors to insert before testing search");
DEFINE_int32(benchmark_duration, 10, "Duration (in seconds) to run each ef benchmark.");
...
DEFINE_int32(batch_size, 1000, "Batch size for search operations");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== Two-Phase Benchmark: Insert then Search ===" << std::endl;
    std::cout << "Phase 1: Insert " << FLAGS_num_vectors_to_insert << " vectors" << std::endl;
    std::cout << "Phase 2: Benchmark search for " << FLAGS_benchmark_duration << " seconds" << std::endl;
    ...
    // Replicate data for more queries
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        if (FLAGS_use_physical_cores_only) {
            if(i < 4) {
                ...
                thread_params[i].core_start = (i-4) * omp_threads_per_worker * 2 + 1;
            }
        } else {
    ...
    std::vector<int> ef_search_values = {48};
    ...
    std::ofstream outfile("../benchs/pipeline/test/insert_then_search_results.txt");
    outfile << "# Two-phase benchmark: " << FLAGS_num_vectors_to_insert << " vectors inserted first" << std::endl;
    ...
    std::ofstream outfile2("../benchs/pipeline/test/insert_then_search_details.txt");
    outfile2 << "# Two-phase benchmark: " << FLAGS_num_vectors_to_insert << " vectors inserted first" << std::endl;
...
void* client_worker(void* param) {
    ...
    std::cout << "\n=== PHASE 1: Inserting " << FLAGS_num_vectors_to_insert << " vectors ===" << std::endl;
    auto insert_start = high_resolution_clock::now();
    
    const int max_insert_batch = 60;  // Max vectors per insert call
    int remaining = FLAGS_num_vectors_to_insert;
    int offset = 0;
    int total_inserted = 0;
    
    while (remaining > 0 && offset * dim_query_data < (int)query_data.size()) {
        int current_insert_batch = std::min(max_insert_batch, remaining);
        current_insert_batch = std::min(current_insert_batch, ((int)query_data.size() - offset * dim_query_data) / dim_query_data);
        
        if (current_insert_batch <= 0) break;
        
        const float* insert_data_ptr = query_data.data() + offset * dim_query_data;
        ...
        total_inserted += current_insert_batch;
        remaining -= current_insert_batch;
        offset += current_insert_batch;
        
        if (total_inserted % 1000 == 0) {
            std::cout << "Inserted " << total_inserted << " / " << FLAGS_num_vectors_to_insert << " vectors" << std::endl;
        }
    }
    
    auto insert_end = high_resolution_clock::now();
    double insert_duration = duration_cast<milliseconds>(insert_end - insert_start).count();
    std::cout << "Insertion complete: " << total_inserted << " vectors in " << insert_duration << " ms" << std::endl;
    std::cout << "Insertion throughput: " << (total_inserted / (insert_duration / 1000.0)) << " vectors/s" << std::endl;

    // --- PHASE 2: BENCHMARK SEARCH ---
    std::cout << "\n=== PHASE 2: Benchmarking Search ===" << std::endl;
    ...
    // --- Lambda: Run a timed search benchmark in batches ---
    auto run_ef_benchmark = [&](int ef, size_t duration_sec) {
        ...
        while (duration_cast<seconds>(high_resolution_clock::now() - bench_start).count() < (long)duration_sec) {
            ...
            for (int i = 0; i < current_batch_size; ++i) {
                ...
                int gt_idx = ((query_index % n_query_data_thread) + i) * dim_ground_truth;
        ...
        // Calculate metrics
        double avg_latency = (queries_executed > 0) ? total_query_time / queries_executed : 0.0;
        double throughput = (queries_executed > 0) ? queries_executed / (total_query_time * 1e-6) : 0.0;
        ...
        std::cout << "  Total queries: " << queries_executed << std::endl;
        std::cout << "  Avg latency per query: " << avg_latency << " us" << std::endl;
        std::cout << "  Throughput: " << throughput << " queries/s" << std::endl;
        ...
        thread_param.per_ef_latencies.push_back(avg_latency);
