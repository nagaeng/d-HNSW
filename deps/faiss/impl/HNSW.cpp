  ...
  VisitedTable &vt)
{
  ...
  vt.set(entry_point);  // Bounds check now inside VisitedTable::set()
  ...
  while (!candidates.empty()) {
    ...
    for(size_t i = begin; i < end; i++) {
      ...
      if (nodeId < 0) break;
      if (vt.get(nodeId)) continue;  // Bounds check now inside VisitedTable::get()
      vt.set(nodeId);
  ...
  int level, int nres_in) const
{
  ...
  for (int i = 0; i < candidates.size(); i++) {
    ...
    FAISS_ASSERT(v1 >= 0);
    // Skip invalid node IDs - bounds check inside VisitedTable::is_valid()
    if (!vt.is_valid(v1)) {
      continue;
    }
    if (nres < k) {
  ...
  while (candidates.size() > 0) {
    ...
    for (size_t j = begin; j < end; j++) {
      int v1 = neighbors[j];
      if (v1 < 0) break;
      if (vt.get(v1)) {  // Bounds check now inside VisitedTable::get()
        continue;
      }
      vt.set(v1);
      ndis++;
