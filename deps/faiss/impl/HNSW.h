...
struct VisitedTable {
  ...
  /// Check if index is valid (bounds check for corrupted indices)
  bool is_valid(int no) const {
    return no >= 0 && no < static_cast<int>(visited.size());
  }

  /// set flag #no to true (with bounds check)
  void set(int no) {
    if (no >= 0 && no < static_cast<int>(visited.size())) {
      visited[no] = visno;
    }
  }

  /// get flag #no (with bounds check - returns true for invalid to skip)
  bool get(int no) const {
    if (no < 0 || no >= static_cast<int>(visited.size())) {
      return true;  // Return true to skip invalid nodes
    }
